<?php
	...
	if ( Session::exists('create_news')){
		echo Session::flash('create_news');
	}
	$errors = array();
	...
	if(Input::get('submit'))
	{
		if( Token::check_token(Input::get('token')) )
		{
			...
			{
				$admin->createnews(array(
					'judul_berita' => Input::get('judul'),
					'news' => Input::get('isi_berita'),
					'date' => date('Y-m-d H:i:s'),
					'author' => Session::get_session('username')
					));
				Session::flash('create_news','Berita Berhasil Di Buat');
				Redirect::To('createnews');
			} else {
...
?>
	...
	<div class="profile">
		...
		<form method="post" action="createnews.php">
